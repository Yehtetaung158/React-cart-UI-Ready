...
const Cart = ({cart:{image,title,price,cost,id,Product_id}}) => {

    const {delCart}=useContext(DataContext)
  ...
  return (
<div className="cart-item group" cart-product-id={1}>
  <div className="w-full mb-5">
    <img className="h-16 -mb-8 ms-3 cart-item-img relative z-10" src={image} />
    <div className="border bg-white border-neutral-600 p-3 relative">
      <button 
      onClick={delCartHandler}
      className="cart-item-del bg-red-100 text-red-600 p-1 duration-200 absolute top-3 -right-3 opacity-0 group-hover:opacity-100 group-hover:right-3 group-hover:pointer-events-auto">
      ...
      <p className="cart-item-title font-heading line-clamp-1 font-bold mt-7 mb-3">{title}</p>
      <div className="flex justify-between">
        <p className="text-neutral-500">
          $ <span className="cart-item-price hidden">{cost}</span>
